...
- **Per-Client Lifecycle Tracking** - Last successful job timestamp per client/policy/action (BACKUP, DUPLICATION, IMPORT) for SLA alerting
- **Multi-Site Support** - Monitor multiple NetBackup master servers with a `site` constant label per instance
...
- **Multi-Version API Support** - Automatic detection of NetBackup API versions (3.0, 12.0, 13.0, 14.0)
...
|-------------------|-------------|----------------|
| 11.2+             | 14.0        | Fully Supported (opt-in collectors: alerts, malware, catalog, SLO) |
| 11.0              | 13.0        | Fully Supported |
| 10.5              | 12.0        | Fully Supported |
| 10.0 - 10.4       | 10.0        | Fully Supported |

The exporter automatically detects the highest supported API version (14.0 → 13.0 → 12.0 → 3.0).
